...
- [x] Camera→screen mapping must preserve the camera's aspect ratio — hand→play coords scale X by the canvas-aspect-dependent `worldWidth` but Y by a fixed `VERT_SPAN=12`, ignoring the camera's aspect, so in non-16:10 viewports (e.g. devtools portrait mobile) motion is anisotropic and some finger points feel impossible; apply ONE uniform world-units-per-camera-unit scale to both axes derived from the camera aspect, default FIT (whole camera reachable), shared by `engine.ts` + `pilot.ts` + `handCursor.ts`, keep mirror/margin/clampHalf; ONLY the flexible scenes (`/pose`,`/characters`,`/harness`,`/keyboard`) — leave battle `/game` 16:10-locked & its mapping untouched — see [camera-aspect-scaling.md](camera-aspect-scaling.md)
